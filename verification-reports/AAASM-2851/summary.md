...
**Note on AAASM-2867 changes to the F1 trigger semantics:** After AAASM-2867 lands, the main-only Bump step rewrites `optionalDependencies['@agent-assembly/runtime-*']` to `${binary_source_tag#v}` and runs BEFORE the pre-flight check. F1's `workspace:*`-based pre-flight assumption (pre-flight reads `package.json` as-pinned-on-master) no longer holds: the pre-flight now reads the post-Bump real-version pins derived from the dispatch's `binary_source_tag`. The captured F1.log remains valid as evidence of fail-fast behavior, but the trigger condition for reproducing the failure shifts — instead of editing `optionalDependencies` on a temp branch, the equivalent reproduction would dispatch with `binary_source_tag` pointing at a tag whose runtime sub-packages were never published on npm (the rewritten `runtimeVersion` then fails the same `npm view` lookup).
