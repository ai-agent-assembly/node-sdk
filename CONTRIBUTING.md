...
Generated content lives between HTML-comment sentinels so the surrounding
prose is untouched:
...
2. Add a `render<Block>(meta)` function in `scripts/generate-docs-metadata.mjs`
   that returns the body of the new bounded block, and register it in the
   `BLOCKS` map at the top of the file.
3. Wrap the target snippet in the relevant `.md` file(s) with matching
   `<!-- BEGIN GENERATED: <id> --> ... <!-- END GENERATED: <id> -->` sentinels.
