// AUTO-GENERATED by scripts/gen-proto.mjs from ../agent-assembly/proto/. Do not edit by hand.
// Code generated by protoc-gen-ts_proto. DO NOT EDIT.
// versions:
//   protoc-gen-ts_proto  v2.11.8
//   protoc               v7.34.1
// source: common.proto

/* eslint-disable */
import { BinaryReader, BinaryWriter } from "@bufbuild/protobuf/wire";

export const protobufPackage = "assembly.common.v1";

/**
 * Decision is the outcome of a policy evaluation. Used by policy.proto and
 * mirrored in audit.proto to keep audit self-contained.
 */
export enum Decision {
  DECISION_UNSPECIFIED = 0,
  /** ALLOW - Action is permitted immediately. */
  ALLOW = 1,
  /** DENY - Action is blocked; no further recourse. */
  DENY = 2,
  /** PENDING - Action is held — a human reviewer must approve or reject it. */
  PENDING = 3,
  /** REDACT - Action is permitted but sensitive fields must be redacted first. */
  REDACT = 4,
  UNRECOGNIZED = -1,
}

export function decisionFromJSON(object: any): Decision {
  switch (object) {
    case 0:
    case "DECISION_UNSPECIFIED":
      return Decision.DECISION_UNSPECIFIED;
    case 1:
    case "ALLOW":
      return Decision.ALLOW;
    case 2:
    case "DENY":
      return Decision.DENY;
    case 3:
    case "PENDING":
      return Decision.PENDING;
    case 4:
    case "REDACT":
      return Decision.REDACT;
    case -1:
    case "UNRECOGNIZED":
    default:
      return Decision.UNRECOGNIZED;
  }
}

export function decisionToJSON(object: Decision): string {
  switch (object) {
    case Decision.DECISION_UNSPECIFIED:
      return "DECISION_UNSPECIFIED";
    case Decision.ALLOW:
      return "ALLOW";
    case Decision.DENY:
      return "DENY";
    case Decision.PENDING:
      return "PENDING";
    case Decision.REDACT:
      return "REDACT";
    case Decision.UNRECOGNIZED:
    default:
      return "UNRECOGNIZED";
  }
}

/**
 * ActionType classifies what kind of operation the agent is attempting.
 * Drives both policy rule matching and audit event categorisation.
 */
export enum ActionType {
  ACTION_UNSPECIFIED = 0,
  /** LLM_CALL - Call a large-language model (e.g. GPT-4o, Claude, Gemini). */
  LLM_CALL = 1,
  /** TOOL_CALL - Invoke a tool or MCP server function. */
  TOOL_CALL = 2,
  /** FILE_OPERATION - Read, write, delete, or create a local file. */
  FILE_OPERATION = 3,
  /** NETWORK_CALL - Make an outbound network request. */
  NETWORK_CALL = 4,
  /** PROCESS_EXEC - Execute a subprocess. */
  PROCESS_EXEC = 5,
  /** AGENT_SPAWN - Spawn a child agent. */
  AGENT_SPAWN = 6,
  /**
   * TOOL_RESULT - Result body returned by a tool invocation — evaluated on the response
   * path so policies can redact secrets in the body before the agent sees it.
   */
  TOOL_RESULT = 7,
  UNRECOGNIZED = -1,
}

export function actionTypeFromJSON(object: any): ActionType {
  switch (object) {
    case 0:
    case "ACTION_UNSPECIFIED":
      return ActionType.ACTION_UNSPECIFIED;
    case 1:
    case "LLM_CALL":
      return ActionType.LLM_CALL;
    case 2:
    case "TOOL_CALL":
      return ActionType.TOOL_CALL;
    case 3:
    case "FILE_OPERATION":
      return ActionType.FILE_OPERATION;
    case 4:
    case "NETWORK_CALL":
      return ActionType.NETWORK_CALL;
    case 5:
    case "PROCESS_EXEC":
      return ActionType.PROCESS_EXEC;
    case 6:
    case "AGENT_SPAWN":
      return ActionType.AGENT_SPAWN;
    case 7:
    case "TOOL_RESULT":
      return ActionType.TOOL_RESULT;
    case -1:
    case "UNRECOGNIZED":
    default:
      return ActionType.UNRECOGNIZED;
  }
}

export function actionTypeToJSON(object: ActionType): string {
  switch (object) {
    case ActionType.ACTION_UNSPECIFIED:
      return "ACTION_UNSPECIFIED";
    case ActionType.LLM_CALL:
      return "LLM_CALL";
    case ActionType.TOOL_CALL:
      return "TOOL_CALL";
    case ActionType.FILE_OPERATION:
      return "FILE_OPERATION";
    case ActionType.NETWORK_CALL:
      return "NETWORK_CALL";
    case ActionType.PROCESS_EXEC:
      return "PROCESS_EXEC";
    case ActionType.AGENT_SPAWN:
      return "AGENT_SPAWN";
    case ActionType.TOOL_RESULT:
      return "TOOL_RESULT";
    case ActionType.UNRECOGNIZED:
    default:
      return "UNRECOGNIZED";
  }
}

/** RiskTier classifies an agent or policy by the risk level it operates at. */
export enum RiskTier {
  RISK_UNSPECIFIED = 0,
  /** LOW - Low risk: log-only enforcement; no blocking. */
  LOW = 1,
  /** MEDIUM - Medium risk: block-and-optionally-approve enforcement. */
  MEDIUM = 2,
  /** HIGH - High risk: always block; human review mandatory. */
  HIGH = 3,
  /** CRITICAL - Critical risk: immediate kill + incident escalation. */
  CRITICAL = 4,
  UNRECOGNIZED = -1,
}

export function riskTierFromJSON(object: any): RiskTier {
  switch (object) {
    case 0:
    case "RISK_UNSPECIFIED":
      return RiskTier.RISK_UNSPECIFIED;
    case 1:
    case "LOW":
      return RiskTier.LOW;
    case 2:
    case "MEDIUM":
      return RiskTier.MEDIUM;
    case 3:
    case "HIGH":
      return RiskTier.HIGH;
    case 4:
    case "CRITICAL":
      return RiskTier.CRITICAL;
    case -1:
    case "UNRECOGNIZED":
    default:
      return RiskTier.UNRECOGNIZED;
  }
}

export function riskTierToJSON(object: RiskTier): string {
  switch (object) {
    case RiskTier.RISK_UNSPECIFIED:
      return "RISK_UNSPECIFIED";
    case RiskTier.LOW:
      return "LOW";
    case RiskTier.MEDIUM:
      return "MEDIUM";
    case RiskTier.HIGH:
      return "HIGH";
    case RiskTier.CRITICAL:
      return "CRITICAL";
    case RiskTier.UNRECOGNIZED:
    default:
      return "UNRECOGNIZED";
  }
}

/**
 * EnforcementMode controls whether policy decisions are applied to agent
 * actions or only observed (dry-run / sandbox mode). Used at both the policy
 * document level and as a per-agent override on RegisterRequest.
 *
 * Resolution order on the gateway, per evaluation:
 *   1. RegisterRequest.enforcement_mode (if set / non-UNSPECIFIED)
 *   2. Policy document enforcement_mode (parsed from YAML body)
 *   3. Server-wide default: ENFORCE
 */
export enum EnforcementMode {
  /** ENFORCEMENT_MODE_UNSPECIFIED - Sentinel; resolves to ENFORCE on the server side. */
  ENFORCEMENT_MODE_UNSPECIFIED = 0,
  /** ENFORCE - Default: deny blocks, redact strips, pending halts execution. */
  ENFORCE = 1,
  /** OBSERVE - Dry-run / sandbox: decisions computed and audited; no enforcement applied. */
  OBSERVE = 2,
  /** DISABLED - Policy evaluation disabled entirely. Only valid in hermetic test environments. */
  DISABLED = 3,
  UNRECOGNIZED = -1,
}

export function enforcementModeFromJSON(object: any): EnforcementMode {
  switch (object) {
    case 0:
    case "ENFORCEMENT_MODE_UNSPECIFIED":
      return EnforcementMode.ENFORCEMENT_MODE_UNSPECIFIED;
    case 1:
    case "ENFORCE":
      return EnforcementMode.ENFORCE;
    case 2:
    case "OBSERVE":
      return EnforcementMode.OBSERVE;
    case 3:
    case "DISABLED":
      return EnforcementMode.DISABLED;
    case -1:
    case "UNRECOGNIZED":
    default:
      return EnforcementMode.UNRECOGNIZED;
  }
}

export function enforcementModeToJSON(object: EnforcementMode): string {
  switch (object) {
    case EnforcementMode.ENFORCEMENT_MODE_UNSPECIFIED:
      return "ENFORCEMENT_MODE_UNSPECIFIED";
    case EnforcementMode.ENFORCE:
      return "ENFORCE";
    case EnforcementMode.OBSERVE:
      return "OBSERVE";
    case EnforcementMode.DISABLED:
      return "DISABLED";
    case EnforcementMode.UNRECOGNIZED:
    default:
      return "UNRECOGNIZED";
  }
}

/**
 * AgentId is a composite identity used in every gRPC message.
 * Using a message (not a flat string) lets the gateway enforce org/team scoping
 * and issue scoped credentials without changing downstream message shapes.
 */
export interface AgentId {
  /** Organisation identifier. */
  orgId: string;
  /** Team within the organisation. */
  teamId: string;
  /** The agent's own unique ID (DID-style; assigned at registration). */
  agentId: string;
}

/** Timestamp uses Unix milliseconds for precision without proto well-known type imports. */
export interface Timestamp {
  unixMs: number;
}

function createBaseAgentId(): AgentId {
  return { orgId: "", teamId: "", agentId: "" };
}

export const AgentId: MessageFns<AgentId> = {
  encode(message: AgentId, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.orgId !== "") {
      writer.uint32(10).string(message.orgId);
    }
    if (message.teamId !== "") {
      writer.uint32(18).string(message.teamId);
    }
    if (message.agentId !== "") {
      writer.uint32(26).string(message.agentId);
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): AgentId {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseAgentId();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
          if (tag !== 10) {
            break;
          }

          message.orgId = reader.string();
          continue;
        }
        case 2: {
          if (tag !== 18) {
            break;
          }

          message.teamId = reader.string();
          continue;
        }
        case 3: {
          if (tag !== 26) {
            break;
          }

          message.agentId = reader.string();
          continue;
        }
      }
      if ((tag & 7) === 4 || tag === 0) {
        break;
      }
      reader.skip(tag & 7);
    }
    return message;
  },

  fromJSON(object: any): AgentId {
    return {
      orgId: isSet(object.orgId)
        ? globalThis.String(object.orgId)
        : isSet(object.org_id)
        ? globalThis.String(object.org_id)
        : "",
      teamId: isSet(object.teamId)
        ? globalThis.String(object.teamId)
        : isSet(object.team_id)
        ? globalThis.String(object.team_id)
        : "",
      agentId: isSet(object.agentId)
        ? globalThis.String(object.agentId)
        : isSet(object.agent_id)
        ? globalThis.String(object.agent_id)
        : "",
    };
  },

  toJSON(message: AgentId): unknown {
    const obj: any = {};
    if (message.orgId !== "") {
      obj.orgId = message.orgId;
    }
    if (message.teamId !== "") {
      obj.teamId = message.teamId;
    }
    if (message.agentId !== "") {
      obj.agentId = message.agentId;
    }
    return obj;
  },

  create(base?: DeepPartial<AgentId>): AgentId {
    return AgentId.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<AgentId>): AgentId {
    const message = createBaseAgentId();
    message.orgId = object.orgId ?? "";
    message.teamId = object.teamId ?? "";
    message.agentId = object.agentId ?? "";
    return message;
  },
};

function createBaseTimestamp(): Timestamp {
  return { unixMs: 0 };
}

export const Timestamp: MessageFns<Timestamp> = {
  encode(message: Timestamp, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.unixMs !== 0) {
      writer.uint32(8).int64(message.unixMs);
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): Timestamp {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseTimestamp();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
          if (tag !== 8) {
            break;
          }

          message.unixMs = longToNumber(reader.int64());
          continue;
        }
      }
      if ((tag & 7) === 4 || tag === 0) {
        break;
      }
      reader.skip(tag & 7);
    }
    return message;
  },

  fromJSON(object: any): Timestamp {
    return {
      unixMs: isSet(object.unixMs)
        ? globalThis.Number(object.unixMs)
        : isSet(object.unix_ms)
        ? globalThis.Number(object.unix_ms)
        : 0,
    };
  },

  toJSON(message: Timestamp): unknown {
    const obj: any = {};
    if (message.unixMs !== 0) {
      obj.unixMs = Math.round(message.unixMs);
    }
    return obj;
  },

  create(base?: DeepPartial<Timestamp>): Timestamp {
    return Timestamp.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<Timestamp>): Timestamp {
    const message = createBaseTimestamp();
    message.unixMs = object.unixMs ?? 0;
    return message;
  },
};

type Builtin = Date | Function | Uint8Array | string | number | boolean | undefined;

export type DeepPartial<T> = T extends Builtin ? T
  : T extends globalThis.Array<infer U> ? globalThis.Array<DeepPartial<U>>
  : T extends ReadonlyArray<infer U> ? ReadonlyArray<DeepPartial<U>>
  : T extends {} ? { [K in keyof T]?: DeepPartial<T[K]> }
  : Partial<T>;

function longToNumber(int64: { toString(): string }): number {
  const num = globalThis.Number(int64.toString());
  if (num > globalThis.Number.MAX_SAFE_INTEGER) {
    throw new globalThis.Error("Value is larger than Number.MAX_SAFE_INTEGER");
  }
  if (num < globalThis.Number.MIN_SAFE_INTEGER) {
    throw new globalThis.Error("Value is smaller than Number.MIN_SAFE_INTEGER");
  }
  return num;
}

function isSet(value: any): boolean {
  return value !== null && value !== undefined;
}

export interface MessageFns<T> {
  encode(message: T, writer?: BinaryWriter): BinaryWriter;
  decode(input: BinaryReader | Uint8Array, length?: number): T;
  fromJSON(object: any): T;
  toJSON(message: T): unknown;
  create(base?: DeepPartial<T>): T;
  fromPartial(object: DeepPartial<T>): T;
}
